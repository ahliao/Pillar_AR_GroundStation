// This code is from the libkovan from kipr at
// https://github.com/kipr/
...
class Socket
{
	public:
		...
		ssize_t recvfrom(void *const buffer, const size_t length, 
				Address &address, int flags = 0);

		ssize_t send(const void *const buffer, const size_t length, int flags = 0);
		ssize_t sendto(const void *const buffer, const size_t length, 
